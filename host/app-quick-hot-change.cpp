...
#define QUICK_SWITCH 1
#define OP_WHEN_SWITCH 1
...
void quick_skew_partition(int partition_ratio_to_dpu, int total_access_num, int tid){
	...
	std::pair<uint64_t, uint64_t> adjust_partition[32]; // dpu_id, partition_id
	...
	// debug
	// for(int hh = 0; hh < real_adjust_num; hh++){
	// 	std::cout << adjust_partition[hh].first << " " << adjust_partition[hh].second << std::endl;
	// }
...
void create_query_zipf_shift(DTYPE * input, DTYPE * querys, uint64_t  nr_elements, uint64_t nr_querys, uint64_t shift_length, size_t* random_seed = nullptr) {
	ScrambledZipfianGenerator zipf_gen(nr_elements, random_seed);
	for (int i = 0; i < nr_querys; i++) {
		uint64_t pos = zipf_gen.nextValue() + shift_length;
		querys[i] = input[pos % nr_elements];
	}
}
					...
					for(int k = 0; k < INTERLEAVE; k++){
						...
						if((host_recv_buffer[tid][i].rbuffer[j] + k < dindex.total_size) && (dindex.keys[host_recv_buffer[tid][i].rbuffer[j] + k] == host_send_buffer[tid][i].sbuffer[j])){ //(host_recv_buffer[tid][i].rbuffer[j] + k < dindex.total_size) &&
							...
							if(type_array[tid][i].ops[j] == 0){
								// GET
								REGET:
								if (lock_arr[pos >> 10].test_lock_set(version)){ // !!!! 9在load 150M时表现好，10在load 200M时表现好。锁的大小导致性能急速下降？
									while(lock_arr[pos >> 10].test_lock_set(version)){
									};
								}
								ret = dindex.payloads[host_recv_buffer[tid][i].rbuffer[j] + k];
								if (lock_arr[pos >> 10].test_lock_version_change(version)){
									goto REGET;
								}
							}else if(type_array[tid][i].ops[j] == 1){
								// UPDATE
								lock_arr[pos >> 10].get_lock();
								dindex.payloads[host_recv_buffer[tid][i].rbuffer[j] + k] = new_val;
								lock_arr[pos >> 10].release_lock();
							}
							not_found = false;
							break;
					...
					for(int k = 0; k < INTERLEAVE; k++){
						if((cur_key_off + k < dindex.total_size) && dindex.keys[cur_key_off + k] == search_key){ //(cur_key_off + k < dindex.total_size) && 
							...
							if(type_array[tid][i].ops[j] == 0){
								// GET
								REGET2:
								if (lock_arr[pos >> 10].test_lock_set(version)){
									while(lock_arr[pos >> 10].test_lock_set(version)){
									};
								}
								ret = dindex.payloads[host_recv_buffer[tid][i].rbuffer[j] + k];
...
void inline merge_and_retrain(){
	...
	[&](size_t thread_id){
		...
		uint32_t end_page_offset = std::min(interleave_page_offset * (tid + 1), (uint32_t)(dindex.num_buf_pages));
		...
		if(tid == (MERGE_THREAD_NUM - 1)){
			total_ops = key_count[tid] + max_old_input_size - cur_old_off + 1;
		}
	...
	// if(std::is_sorted(new_keys, new_keys + new_total_size))
...
int main(int argc, char **argv) {
...
#if QUICK_SWITCH
	DPU_ASSERT(dpu_alloc(NR_DPUS, "nrThreadPerPool=8", &dpu_set)); // quick switch 开启!!!
#else
	...
	if(is_insert){
		...
		if (sample_distribution == "uniform") {
			...
			for(int i = 0; i < num_querys; i++){
				...
				else if (prob < 0.55){ //UPDATE 0.55(0.5 get, 0.05 update, 0.45insert, >>9 lock, 性能较好)
					int pos = zipf_gen.nextValue();
					querys[i] = keys[pos];
					ops[i] = 1;
				}else{
	...
	// create_query(keys, querys, elements_num, num_querys);
	size_t newseed = 1866;
	create_query_zipf_shift(keys, querys, input_size, num_querys, 20000000);
	// create_query_zipf(keys, querys, input_size, num_querys);
	...
	// // partition_access_level.calculate_partition_skew(NR_DPUS / NR_PARTITION, 1000000, wtid);
	// // load_hot_replica(wtid);
	// quick_skew_partition(NR_DPUS / NR_PARTITION, 1000000, wtid);
	// end_time = tn.rdtsc();
	// diff = tn.tsc2ns(end_time) - tn.tsc2ns(start_time);
	// std::cout << "hot switch time (second) " << (diff/(double) 1000000000) << std::endl;
	...
	if(is_insert){
		...
		for(int i = 0; i < THREAD_NUM; i++){
			...
			RETRY:
				...
				/** judge Load Balance !!! **/
				if(not_change && (buffer_size.first / 8) > (1.5 * (end_ops - start_ops) / NR_DPUS)){
					//Find load unbalance
				...
				// (2) get new hotspot info
					...
					hot_end_time = tn.rdtsc();
					auto hot_diff = tn.tsc2ns(hot_end_time) - tn.tsc2ns(hot_start_time);
					std::cout << "hot switch time (second) " << (hot_diff/(double) 1000000000) << std::endl;
					not_change = false;
					// (4)release lock
				...
				// std::cout << "after max buffer size " << buffer_size.first / 8 << " expected buffer size " << (end_ops - start_ops) / NR_DPUS << std::endl;
	...
	auto diff2 = tn.tsc2ns(end_time) - tn.tsc2ns(hot_end_time);
	std::cout << "after change throughput (wo hot switch) " << num_querys / (diff2/(double) 1000000000) << std::endl;
